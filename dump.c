...
static char* whatFuncDefStyle(FuncDefStyle func_def) /* style of function definition */
{
    switch(func_def)
    {
        case FUNC_NONE:
...
void dump_parameter(Parameter* p, int level)
{
    dump_declarator(p->declarator, level + 1);
    dump_decl_spec(&(p->decl_spec), level + 1);
}

void dump_param_list(ParameterList* p, int level)
{
    struct parameter* q;
...
void dump_declarator(Declarator* d, int level)
{
    PAD;
    Trace("%sdeclarator %p\n", pad, d);
    Trace("%s   name /%s/\n", pad, d->name);
...
static char* flagsDeclSpec(int flags)
{
    ...
    static struct
    {
        ...
        { DS_EXTERN, "extern" }, { DS_STATIC, "static" }, { DS_CHAR, "char" },
        { DS_SHORT, "short" },   { DS_FLOAT, "float" },   { DS_JUNK, "junk" },
    };
    unsigned j;
    *temp = 0;
...
void dump_decl_spec(DeclSpec* d, int level)
{
    PAD;
    Trace("%sdecl_spec %p\n", pad, d);
