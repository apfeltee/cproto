...
#if !HAVE_LIBDMALLOC
    #ifdef NO_LEAKS
void* xMalloc(size_t n, char* f GCC_UNUSED, int l GCC_UNUSED)
    #else
...
#if !HAVE_LIBDMALLOC
    #ifdef NO_LEAKS
void* xRealloc(void* p, size_t n, char* f GCC_UNUSED, int l GCC_UNUSED)
    #else
 ...
 */
static void parse_options(char* src, int maxargc, int* pargc, char** argv)
{
    char* g;
    char* p;
    char c;
 ...
 */
static void add_inc_dir(const char* src)
{
...
static void add2list(char* dst, char* src)
{
    if(*dst)
        strcat(dst, ",");
    strcat(dst, src);
...
static void add_option(char* keyword, char* src)
{
    if(*src)
...
static int quote_length(char* s)
{
    int len = strlen(s);
 ...
 */
static void process_options(int* pargc, char*** pargv)
{
    int argc;
    int eargc;
    int nargc;
    char** argv;
    char* eargv[MAX_OPTIONS];
    char** nargv;
    int i;
    int c;
    char* s;
    ...
    while((c = getopt(argc, argv, ALL_OPTIONS)) != EOF)
    {
        switch(c)
        {
            ...
            case 'P':
                ...
                i = (c == 'C') ? FMT_FUNC_COMMENT : ((c == 'F') ? FMT_FUNC : FMT_PROTO);
...
int main(int argc, char* argv[])
{
    ...
    {
        if(progname[i] == SQUARE_R || progname[i] == ':')
        {
            progname += (i + 1);
            break;
        }
    ...
    else
    {
        ...
        #define FileName argv[i]
            ...
            if(func_style == FUNC_NONE && cpp != NULL)
            {
                 ...
                 */
                sprintf(cpp_cmd, cpp, call_mktemp(strcpy(temp, "sys$scratch:XXXXXX.i")));
