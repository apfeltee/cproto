 ...
 */
void new_decl_spec(DeclSpec* decl_spec, const char* text, long offset, int flags)
{
#if OPT_LINTLIBRARY
    if(lintLibrary())
    {
...
static char* glue_strings(char* a, char* b)
{
    char* result;
 ...
 */
void check_untagged(DeclSpec* decl_spec)
{
 ...
 */
Declarator* new_declarator(const char* text, const char* name, long offset)
{
    Declarator* d;
 ...
 */
Parameter* new_parameter(DeclSpec* decl_spec, Declarator* declarator)
{
    Parameter* param;
    param = NEW(Parameter);
 ...
 */
static boolean is_void_parameter(Parameter* p)
{
    return (boolean)((p == NULL) || (strcmp(p->decl_spec.text, "void") == 0 && p->declarator->text[0] == '\0'));
}
 ...
 */
void add_ident_list(ParameterList* to, ParameterList* from, const char* name)
{
    Parameter* p;
 ...
 */
void set_param_types(ParameterList* params, DeclSpec* decl_spec, DeclaratorList* declarators)
{
    Declarator* d;
    Parameter* p;
 ...
 */
static int put_parameter(FILE* outf,
                         Parameter* p,
                         ...
                         int commented) /* comment-delimiters already from higher level */
{
    ...
    if(lintLibrary())
    {
        ...
        else if(p->declarator->name[0] == '\0')
        {
            ...
            { /* e.g., s is "[20]" for "char [20]" parameter */
    ...
    if(p->declarator->text[0] != '\0')
    {
        ...
        {
            if(proto_style != PROTO_ABSTRACT || proto_comments || where != FUNC_PROTO
 ...
 */
static void put_param_list(FILE* outf, Declarator* declarator, int commented)
{
    int count = 0;
    Parameter* p;
    int f;
    ...
    else
    {
        ...
        if(where == FUNC_PROTO && knrLintLibrary() && (func_declarator != declarator))
        {
 ...
 */
static void put_parameters(FILE* outf, Declarator* declarator, int commented)
{
    Parameter* p;
    ...
    else
    {
        ...
        if(where == FUNC_PROTO && proto_style == PROTO_TRADITIONAL && declarator == func_declarator)
        {
            if(proto_comments)
            {
                put_string(outf, COMMENT_BEGIN);
 ...
 */
static void put_func_declarator(FILE* outf, Declarator* declarator, int commented)
{
    char* s;
    char* t;
    char* decl_text;
    ...
    if(declarator->func_stack->func_def == FUNC_NONE)
    {
        ...
        else
        {
            ...
            *t = declarator->name[0];
            ...
            if(where == FUNC_PROTO && proto_style == PROTO_ABSTRACT && declarator != func_declarator)
            {
                if(proto_comments)
                {
                    if(star)
    ...
    *t = '\0';
    ...
    if(where == FUNC_PROTO && (func_declarator == declarator || func_declarator == declarator->head) && proto_macro)
    {
...
static void put_declarator(FILE* outf, Declarator* declarator, int commented)
{
    ...
    {
        if(where == FUNC_PROTO && proto_style == PROTO_ABSTRACT && declarator->name[0] != '\0')
        {
            ...
            *s = '\0';
            if(proto_comments)
            {
                fprintf(outf, "%s%s%s%s%s", declarator->text, COMMENT_BEGIN, declarator->name,
                        COMMENT_END, s + strlen(declarator->name));
            }
            else
            {
                fprintf(outf, "%s%s", declarator->text, s + strlen(declarator->name));
            }
            *s = declarator->name[0];
        }
        else
 ...
 */
static void put_decl_spec(FILE* outf, DeclSpec* decl_spec)
{
    ...
    if(extern_out && !(decl_spec->flags & DS_STATIC) && strkey(decl_spec->text, "extern") == NULL)
    {
...
static void put_llib_params(Declarator* declarator, int commented)
{
    Parameter* p;
    int count = 0;

    nestedParams++;
    p = (declarator->func_stack->func_def != FUNC_NONE) ? declarator->func_stack->params.first :
 ...
 */
void gen_declarations(DeclSpec* decl_spec, /* declaration specifier */
 ...
 */
static int uses_varargs(Declarator* declarator)
{
    Parameter* p;
 ...
 */
static void check_void_param(Declarator* declarator)
{
    DeclSpec decl_spec;
    Parameter* p;
 ...
 */
static void set_param_decl_spec(Declarator* declarator)
{
    Parameter* p;

    for(p = declarator->params.first; p != NULL; p = p->next)
    {
        if(p->decl_spec.text[0] == '\0' && strcmp(p->declarator->text, ELLIPSIS) != 0)
        {
            free(p->decl_spec.text);
 ...
 */
void gen_prototype(DeclSpec* decl_spec, Declarator* declarator)
{
    Parameter* p;
    int commented = FALSE;
 ...
 */
void gen_func_declarator(Declarator* declarator)
{
 ...
 */
static void put_param_decl(Declarator* declarator, int commented)
{
    int count = 0;
    Parameter* p;
 ...
 */
void gen_func_definition(DeclSpec* decl_spec, Declarator* declarator)
{
    Parameter* p;
     ...
     */
    func_declarator = declarator->head;
    if(func_declarator->func_def == func_style || uses_varargs(func_declarator))
    {
        return;
    }
    ...
    if(func_declarator->func_def == FUNC_TRADITIONAL || func_declarator->func_def == FUNC_BOTH)
    {
