...
#ifdef apollo
static int contains(char* ref, char* tst)
{
...
void fail_alloc(char* msg, char* ptr)
{
    Trace("%s: %p\n", msg, ptr);
...
static int record_freed(char* ptr)
{
    register int j;
    if((j = FindArea(ptr)) >= 0)
    {
...
static int record_alloc(char* newp, char* oldp, unsigned len)
{
    register int j;
 ...
 ************************************************************************/
#if DOALLOC
void* doalloc(void* oldp, unsigned amount)
{
    char* newp;
