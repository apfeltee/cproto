 ...
 */
void put_string(FILE* outf, const char* s)
{
    if(*s != '\0')
    {
        fputs(s, outf);
        if(outf == stdout)
        { /* ensure we aren't doing temp-file! */
            while(*s != '\0')
            {
                if(*s++ == '\n')
                    blank_lines++;
 ...
 */
void put_padded(FILE* outf, const char* s)
{
    put_string(outf, s);
...
static char* strip_name(char* s)
{
    ...
    if(strlen(s) > len && !strncmp(s, stripped, len))
        s += len;
    ...
    if(!standard)
    {
        ...
        if(*s == '.' && is_path_sep(s[1]))
            s += 2;
        else if((t = strstr(s, GccLeaf)) != 0 && (t = strstr(t, IncLeaf)) != 0)
        {
...
static char* get_inc_stack(unsigned n)
{
    return (((int)n) < 0 || n >= inc_depth) ? 0 : inc_stack[n];
}

    #ifdef DEBUG
static void dump_stack(char* tag)
{
    unsigned j;
    printf("/* stack%s:%s", tag, cur_file_name());
    for(j = 0; j <= in_include; j++)
        printf("\n\t%d%s:%s", j, j == base_level ? "*" : "", get_inc_stack(j) ? get_inc_stack(j) : "?");
    printf(" */\n");
}
    #endif /* DEBUG */

static void free_inc_stack(unsigned n)
{
...
static void make_inc_stack(unsigned n, char* s)
{
    if(n != 0)
    {
...
void track_in(void)
{
    ...
    else if(!same_file(old_file, cur_file_name()))
    { /* continue/unnest ? */
        unsigned n;
        int found;
 ...
 */
static void add2implied_buf(const char* s, int append)
{
...
void copy_typedef(const char* s)
{
    if(!strcmp(s, "/*") || *s == '#')
        ; /* ignore */
    else if(in_typedef)
...
void imply_typedef(const char* s)
{
 ...
 * lint-library
 */
int lint_ellipsis(Parameter* p)
{
    return (knrLintLibrary() && (!strcmp(p->declarator->name, ELLIPSIS)));
}
 ...
 */
void ellipsis_varargs(Declarator* d)
{
    int count;
    Parameter* p;
 ...
 * function pointers.
 */
int is_actual_func(Declarator* d)
{
              ...
              Declarator* declarator)
{
    ...
    if(is_actual_func(declarator))
    {
        ...
        else if(!strcmp(spec_text, "void") && declarator->text[0] != '*'
    ...
    {
        if(proto_style == PROTO_LINTLIBRARY || proto_style == PROTO_ANSI_LLIB)
        {
            /* SVR4 lint complains if we declare const data without an
             * initializer.
             */
            if(strkey(decl_spec->text, "const") != NULL || strkey(declarator->text, "const") != NULL)
            {
                put_string(outf, " = {0}");
            }
        }
