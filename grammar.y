...
%token <text>    T_IDENTIFIER T_TYPEDEF_NAME T_DEFINE_NAME

    /* storage class */
%token <text>    T_AUTO T_EXTERN T_REGISTER T_STATIC T_TYPEDEF
    /* This keyword included for compatibility with C++. */
%token <text>    T_INLINE
    /* This keyword included for compatibility with GCC */
%token <text>    T_EXTENSION

    /* type specifiers */
%token <text> T_CHAR T_DOUBLE T_FLOAT T_INT T_VOID
%token <text> T_LONG T_SHORT T_SIGNED T_UNSIGNED
%token <text> T_ENUM T_STRUCT T_UNION
    
/* C9X new types */
%token <text> T_Bool T_Complex T_Imaginary

/* type qualifiers */
%token <text> T_TYPE_QUALIFIER
...
function_definition:
    ...
    opt_declaration_list T_LBRACE
    {
         ...
         */
        if (func_style == FUNC_TRADITIONAL && haveAnsiParam() && $2->head->func_def == func_style)
        {
